    ...
    if(isset($_POST['action'])&&$_POST['action']=="wpfrtp_save_post")
        ...
        if($_POST['user_post_title'])
			...
			*/
			...
			//error_log("Start update:".$tmp_title);
			...
			$post['post_title'] 	= $tmp_title;
			$post['post_status'] 	= $post_status;
			$post['post_content'] 	= $tmp_content;				 
			$post['post_category']	= $tmp_categorymulti;
			$post['post_excerpt' ]	= $tmp_excerpt;
		...
		$tmp_page_id 		= intval(isset($_POST['frontier_return_page_id']) ? $_POST['frontier_return_page_id'] : 0 );
		...
		if ( $tmp_cat_return_id != 0 && !isset($_POST['user_post_save']) )
			{
		...
		//error_log("Return link:".$tmp_return);
